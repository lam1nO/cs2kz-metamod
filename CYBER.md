...
- **HUD**: единый `!hud` (Стандартный/MHUD/Выкл, преф `hudType`), per-element тумблеры с
  ...
  - **Обновлённый** (деф., преф `hudTimerStyle=HUD_TIMER_STYLE_UPDATED`, пункт меню
    ...
    showpos) + **нижняя панель** в plain centre-канале (HUD_PRINTCENTER, не HTML) со
    строкой CP/TP (гейт hudCpTp); сопровождает HTML-панель (needHtml), шлётся по
    изменению слепка `BottomPanelState` + heartbeat 1с (канал надёжный, каждый тик не
    слать).
  ...
  - **Плайн-худ спектатора под открытым Html-меню** (с 5d6d02c, любой стиль): владельцу
    ...
    plain-text худ наблюдаемого: скорость(+престрейф в скобках)/время/клавиши;
...
- **Развод верхнего и нижнего оверлеев.** HTML-панель (или открытое Html-меню) и нижняя
  ...
  `CountHtmlLines` по готовому тексту, для меню — `g_pMenus->GetItemCount` + обвязка)
  → `BottomPadLines` → `BottomPanelState.padLines` (часть слепка: смена высоты верха
  обязана перерисовать низ). Пороги `KZ_HUD_BOTTOM_PAD_*` **эмпирические** — реальная
  высота зависит от кегля/языка/разрешения клиента, и серверу она недоступна; правятся
  по живой проверке, а не расчётом.
...
- **Фразы ищутся по плоскому индексу, а не `KeyValues::FindKey`.** `FindKey` трактует
  ...
  `Cleanup` — очистить до `delete translationKV`. Не возвращать `FindKey` при мёрже.
