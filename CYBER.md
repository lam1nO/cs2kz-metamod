...
- **HUD**: единый `!hud` (Стандартный/MHUD/Выкл, преф `hudType`), per-element тумблеры с
  ...
  - **Плайн-худ спектатора под открытым Html-меню** (с 5d6d02c, любой стиль): владельцу
    ...
    plain-text худ наблюдаемого: скорость(+престрейф в скобках)/время/клавиши, три
    строки; `BottomPanelState.menuOpen` — часть слепка (закрытие меню обязано
    перерисовать/стереть низ, даже если остальные поля совпали).
