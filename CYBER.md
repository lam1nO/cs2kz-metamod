...
- **Компакт-худ (преф `compactPanel`) — это ВЁРСТКА, а не отдельный набор элементов.** Он
  ...
  целиком» это `hudType=Off`, а компакт таким рычагом не притворяется. Нижняя панель CP/TP
  при этом останется: это отдельный канал со своим тумблером `hudCpTp`, и `DrawPanels` зовёт
  `UpdateBottomPanel` без гейта на компакт.
