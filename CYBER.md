...
- **Пункт меню — с заглавной буквы**, включая непереводимые термины:
  `Paint`, `Overlap`, `MHUD`. Строчная в начале пункта — баг (репорт 25.07 по `overlap`).
- **Термины оставляем латиницей** там, где так решил пользователь (paint/overlap/showpos),
  но регистр — как у обычного пункта.
- **Статистика прыжков — строго английская** и в чате, и в консоли (решение 24.07):
  ru-переводы jumpstat-фраз не добавлять.
- Названия элементов худа в меню: «Стандартный HUD» / «MHUD» — одно написание везде.
- Палитра стандартного HTML-худа фиксированная (`KZ_HUD_C_*` в `kz_hud.cpp`);
  префы `mhud*Color` — только для particle-MHUD, в HTML-путь их не тянуть
