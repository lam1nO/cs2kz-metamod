...
- **Невидимки — режим слежки за читерами, v2** (`src/kz/invisible/kz_invisible.*`):
  ...
  **КОНТРОЛЛЕРА** + защитно observer pawn — строго пока он в команде спектаторов,
  ...
  SetAngles/Teleport. Смена флага пишет `invisible_flag … state=on|off reason=list|reset` —
  по ней в Loki видно, мигает ли САМ ФЛАГ (значит, список на платформе пишут двое) или
  флаг стоит, а мигает картинка у зрителя.
  Текст `Invisible - Active` описывает ИМЕННО v2 (TAB+`!specs`+тихий вход, только в
  спектаторах) — прежний обещал скрытие модели/звуков от v1, и это стоило ложного багрепорта
  «невидимка не работает» от тестера, проверявшего её живым игроком.
  **ОТКРЫТЫЙ БАГ (репорт 05.08, причина НЕ установлена):** у зрителя строка невидимки в TAB
  мигает, скорборд дёргается. Что уже отброшено: флап списка (флаг мигал бы с чат-анонсом и с
  `invisible_flag` в логе; обе стороны платформы рендерят файл одной функцией байт-в-байт);
  гипотеза «`NetworkStateChanged` возвращает сущность в снапшот» — её опровергает собственный
  `!hide`: он чистит биты ПЕШЕК, которые движение грязнит десятки раз за тик, и не мигает.
  Живые подозреваемые, между которыми надо разводить на канарейке: (1) ранго-скорбордный тракт
  — `ServerRankRevealAll` бродкастом всем каждые 64 тика (≈2 Гц — ровно та каденция, которую
  человек называет «дёргается») + тоггл хвостового пробела в ИМЕНИ игрока в `Player::SetClan`
  (пересортировка TAB); (2) соседние CSSharp-плагины флота (GG1, скины, presence), тоже
  трогающие имя/клантег/команду; (3) сам подход v2 — CS2 может не считать очищенный бит
  контроллера окончательным (прецедентов скрытия КОНТРОЛЛЕРА ни в апстриме, ни в вендорах нет,
  все чистят только пешки/оружие/партиклы). Разведка: `kz_profile_rating_badge_enabled 0` гасит у тракта (1) бродкаст и записи
  в контроллеры, но НЕ тоггл имени — `Player::SetClan` живёт своим путём (`UpdateClantag`
  из смены режима/стиля и из ответа платформы по рейтингу, период 120 с ±30 на игрока и
  +5 с после финиша). Поэтому при выключенном cvar'е переименования и пересортировка TAB
  продолжатся, и «всё ещё мигает» НЕ означает «тракт (1) невиновен» — это ложный минус.
  Чтобы замер был честным: не трогать `!mode`/стили во время наблюдения и держать окно
  короче 120 с, либо гейтить `SetClantag` отдельно. Уже доехавшие клиентам цифры ранга
  cvar не убирает — судить по мельканию, а не по наличию цифр.
- **Невидимка — ТОЛЬКО из спектаторов, это решение пользователя (06.08), а не побочка v2.**
  Гейт `GetTeam() != CS_TEAM_SPECTATOR` в `kz_quiet.cpp` — намеренный: слежка за читерами
  ведётся из наблюдателей, живому игроку невидимость не нужна и не даётся. Снимать гейт
  нельзя. Фраза в чате обязана обещать ровно это (в v1 она обещала невидимость живому
  игроку, звуки и join/leave — и на этом тестер потерял время, решив, что режим сломан).
