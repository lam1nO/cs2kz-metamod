...
static void test_no_teleports()
{
	...
	CutResult r = ComputeAwrCut(v.data(), v.size(), nullptr, 0, 5000, TI);
...
static void test_single_tp()
{
	...
	CutResult r = ComputeAwrCut(v.data(), v.size(), nullptr, 0, 60 * 1000 / 64, TI);
...
static void test_repeat_tp_same_cp()
{
	...
	CutResult r = ComputeAwrCut(v.data(), v.size(), nullptr, 0, 10000, TI);
...
static void test_pause_overlap_not_double_counted()
{
	...
	CutResult r = ComputeAwrCut(v.data(), v.size(), &pause, 1, 10000, TI);
