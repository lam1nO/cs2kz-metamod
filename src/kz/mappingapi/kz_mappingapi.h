...
namespace KZ::course
{
	...
	// Полный список дескрипторов карты, ВКЛЮЧАЯ отключённые платформой. g_sortedCourses
	// отключённые не держит (отключение = исчезновение из !courses), а меню редактора зон
	// обязано показывать и их — иначе отключённый курс нельзя включить обратно из игры.
	// Индекс валиден в пределах одного состояния списка; между кадрами его не хранить.
	u32 GetCourseDescriptorTotal();
	const KZCourseDescriptor *GetCourseDescriptorByIndex(u32 index);
