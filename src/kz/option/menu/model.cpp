		...
		for (const KZOptItem &item : node->items)
		{
			...
			switch (item.type)
			{
				...
				case KZOptItemType::Color:
					...
					// упакованный int, тот же формат, что и GetMHUDColorPref/hud/layout/menu.cpp.
					opts->SetPreferenceInt(item.prefKey,
										   ((i64)item.cdef.r() << 24) | ((i64)item.cdef.g() << 16) | ((i64)item.cdef.b() << 8) | (i64)item.cdef.a());
					break;
