...
void CybAwrBackfill::OnMapChanged()
{
	...
	g_autoDue = false;
	// Сброс точки отсчёта таймера на смену карты. САМ ДЕФЕКТ ctimer починен (кламп часов,
	// пошедших назад, в ProcessTimerList) — прежняя подпись этой строки утверждала, что
	// realtime на changelevel не обнуляется в отличие от curtime, и это НЕВЕРНО: обнуляются
	// обе шкалы, что и доказано замером 08.09 на srv-5 (память проекта
	// fork-timers-stall-after-map-change). Присваивание оставлено как явное «период
	// автоподбора считаем от новой карты» — по смыслу оно в паре с g_autoDue выше, а не
	// страховка от чужого бага.
	if (g_timer)
	{
