...
namespace KZ::replaysystem::data
{
	...
	// событиям; сабтики этот тракт вообще не распаковывает (см. ReadTickDataSkipSubticks),
	// поэтому потолка у них больше нет. Ориентир: самый длинный
	...
	// 64 тик/с, двукратный запас к самому длинному мыслимому рану.
	//
	// Почему отдельной границей, а не байтами: смысл проверки — отсечь абсурд из мусорной
	// шапки (там elementCount доходит до 4 млрд, то есть до терабайтов запроса), а не
	// выгадать десятки мегабайт на легальных файлах. Отказ здесь необратим: воркер бэкфилла
	// отправляет на него `awrMs: null`, и файл больше никогда не вернётся в бэклог.
	...
	// легальном файле. (Раньше эта граница держала ещё и сабтики: 868 Б на кадр
	// (`MAX_SUBTICK_MOVES = 36`) — у двухчасового рана 381 МБ, и байтовый потолок отверг бы
	// его с ~84 минут. Теперь сабтики не распаковываются вовсе, и этой заботы нет.)
	...
	CutSource LoadCutSourceFromMemory(const char *data, size_t size)
	{
		...
		// Секция сабтиков идёт сразу за тиками. Разрезу и `!lead` она НЕ НУЖНА, а стоит она
		// 868 Б на кадр против 264 Б у самих тиков — три четверти пика памяти разбора (на
		// двухчасовом ране ~400 МиБ на ОДИН файл, а файлов в полёте до N, см.
		// kz_awr_backfill_concurrency). Поэтому её здесь не распаковывают вовсе:
		// ReadTickDataSkipSubticks двигает курсор по compressedSize из её шапки.
		//
		// Заодно отпали и её проверки (elementCount, uncompressedSize, делимость для v<4):
		// они защищали аллокации по числам ИЗ ФАЙЛА, а аллокаций в этой секции больше нет.
		// Границу буфера держит сам пропуск. Побочно это перестало отвергать древние файлы
		// без сабтиков (v<4 с elementCount == 0) — для них разрез теперь считается.
		if (!KZ::replaysystem::compression::ReadTickDataSkipSubticks(cursor, end, out.ticks, out.header.version()))
		{
			return out;
		}
