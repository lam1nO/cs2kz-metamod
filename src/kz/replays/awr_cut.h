...
namespace KZ::replaysystem::awr
{
	...
	struct CutResult
	{
		...
		// ИНВАРИАНТ ПРАВИЛА, выраженный числом: максимальное ПОПАРНОЕ расстояние между точками
		// ПРИБЫТИЙ внутри ОДНОГО выреза (юниты) и номер выреза, где оно достигнуто. Правило
		// «вырез = петля ОДНОГО чекпоинта» ровно это и утверждает, поэтому величина обязана
		// быть ≤ AWR_SAME_DEST_TOLERANCE при любом входе. Превышение означает, что правило
		// нарушено НАШИМ кодом (данные его нарушить не могут — вырезы строит разрез), поэтому
		// вызывающий пишет об этом `error`, но файл НЕ отвергает: ошибка наша, а реплей должен
		// остаться играбельным.
		//
		// Зачем именно она: живой пересчёт 17 680 файлов показал, что «дельта числа вырезов»
		// различителем НЕ является (разбор kz_devon main ckz: 9 вырезов и до правки, и после),
		// а вот вырезов, склеивающих прибытия дальше допуска, было ровно 1 до правки (точки в
		// 15.843 u) и 0 после. То есть эта величина прямо утверждает правило, а число вырезов —
		// нет.
		float maxDestSpread = 0.0f;
		uint32_t maxDestSpreadCut = 0;
