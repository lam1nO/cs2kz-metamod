...
namespace KZ::replaysystem::playback
{
	...
	// Проверить инвариант правила разреза «член сшитой цепочки лежит в допуске от её якоря»
	// (awr::CutResult::maxChainDestOffset) и, если он нарушен, написать в лог `error` с
	// машинной причиной. ЕДИНСТВЕННОЕ место, где живут порог и формат строки: копия быстро
	// разъехалась бы, а по этой строке принимают пересчёт. Возвращает true при нарушении,
	// чтобы вызывающий мог решить, что делать с файлом (бэкфилл — мягкий отказ, плейбек —
	// только лог). Зовётся с ГЛАВНОГО потока: из рабочего потока воркера логи не пишем.
	//
	// Перегрузка по плоским полям — для бэкфилла: там результат разбора уже переложен в
	// WorkerResult и самого CutResult на главном потоке нет.
	bool LogDestOffsetViolation(const char *uuid, bool measured, float offset, u32 offsetCut, size_t deadCount);
	bool LogDestOffsetViolation(const char *uuid, const awr::CutResult &cut);
