...
namespace KZ::replaysystem::awr
{
							 ...
							 DestProbe *probe)
	{
		...
		for (int64_t j = (int64_t)t - 1; j >= (int64_t)runStart; j--)
		{
			...
			// Участок пребывания НЕ имеет права перешагнуть прибытие на ДРУГУЮ точку: между
			// разными точками лежит переход (игрок сдвинулся и поставил новый чекпоинт), и он
			// ЖИВОЙ. Именно здесь терялась граница между чекпоинтами: соседние чекпоинты
			// стоят в единицах юнитов друг от друга (замер: 8.9 u), это ВНУТРИ
			// AWR_DEST_TOLERANCE, поэтому «непрерывное пребывание в радиусе» затягивало в
			// вырез и переход, и весь предыдущий кластер петель. На живом файле 01a06ca3 так
			// склеилось 21 прибытие в один вырез на 70.4 с, а стык вырезки оказался между
			// РАЗНЫМИ чекпоинтами (разрыв 18.1 u и 170.9° по yaw).
			//
			// Прибытие на ТУ ЖЕ точку якорь проходит свободно — на этом держится схлопывание
			// повторных попыток одного чекпоинта в один вырез.
							...
							uint32_t runStart, uint32_t runEnd, std::vector<ArrivalTrace> *trace)
	{
		...
		// (замер на 01a06ca3: 18.1 u и 170.9° по yaw). Мёртвое время от слияния не меняется:
		// длительности считаются в кадрах и складываются.
		{
			struct DeadCut
			{
				Interval iv;
			...
			for (size_t i = 0; i < cuts.size(); i++)
			{
				...
				// кадров между ними (фикстура ревью: реальный код даёт [11,21]+[22,60], без
				// сверки — [11,60]). На awrMs это не влияет (кадры те же), но влияет на
				// ЧИСЛО вырезов, а по нему принимают пересчёт и читают трассу.
				//
