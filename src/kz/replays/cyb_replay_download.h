 ...
 *
 * PRO (`WRPro`/`PBPro`, они же `!replay wrpro`/`pbpro`/`gpbpro`): с 26.09 новый
 * PRO-PB выгружается отдельной строкой type=pbpro (submission.cpp,
 * TryUploadCentralReplay) — в том числе когда NUB-PB держит более быстрый TP-ран.
 * Для истории до этого api отдаёт pb-файл, только если лучший overall-ран игрока
 * сам без телепортов (NUB-PB и PRO-PB — один ран). Иначе честный 404: показать
 * ран с ТП под видом pro-рекорда хуже, чем сказать «нет».
 *
