...
private:
	...
	// Молчаливая загрузка уже получила отказ под ТЕКУЩИЙ ключ (карта+режим+курс): без защёлки
	// проверка курса раз в 32 тика ходила бы в сеть вечно на курсе без AWR-реплея. Снимается
	// сменой ключа (режим/курс/карта) и явным `!lead`.
