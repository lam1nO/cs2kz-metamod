...
void KZLeadService::Toggle(CybReplayDownload::Kind kind)
{
	...
	if (this->loading || this->pending)
	{
		...
		// Идущая загрузка ушла под ДРУГОЙ курс/режим или вид записи (её начал процент — AWR, а
		// игрок с тех пор сменил курс или просит `!lead pb`). Подхватить её значило бы поднять луч по чужому маршруту и отчитаться
...
void KZLeadService::OnPathLoaded(std::vector<Vertex> &&newPath)
{
	...
	// Вид — тот же, что у запроса (как и ключ): по нему Toggle отличает `!lead pb` от живого
	// AWR-пути процента, а сравнение решает, можно ли взять эти вершины себе копией.
...
// Вид записи в ключ пути сравнения входит через слот: SetCompareWanted со сменой kind
// освобождает путь и протухает загрузку, так что c.path всегда построен под c.kind.
