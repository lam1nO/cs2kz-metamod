...
// Окно подтверждения (сек realtime): повтор ТОГО ЖЕ действия в его пределах = подтверждение.
// realtime, не игровое время: игрок жмёт клавишу по настенным часам, и на паузе окно тоже
// должно истекать. Защита ВОЗОБНОВЛЯЕМАЯ: не подтвердил — через секунду простоя она снова
// взведена, и следующее одиночное нажатие опять только предупредит (решение пользователя
// 21.08; прежний вариант тратил ровно одно предупреждение на ран и дальше пропускал молча).
...
{
	RESET_CONFIRM_RESTART = 0, // !r/!course/!main/!bN и меню !courses (воронка TeleportToCourse)
...
public:
	...
	// остальных нет.
	bool CheckSafeguard(ResetConfirmAction action, bool showError = true);
