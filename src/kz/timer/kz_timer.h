...
// таймер не набрал столько, !r при включённом !sg проходит. Терять тут нечего — забег ещё не
// начался по существу, а сейфгард в этот момент только мешает перезаходить на старт. Дальше
// при включённом !sg рестарт запрещён наглухо (решение пользователя 21.08: раньше он проходил
// по двойному тапу, то есть !sg рестарт вообще не блокировал). Окно — ТОЛЬКО у !r: noclip и
// чекпоинт-ТП гейтятся своими флагами с первого тика.
...
// рана): после него сброс таймера не проходит молча, а просит подтверждения.
#define KZ_RESET_CONFIRM_MIN_RUNTIME 300.0f
// Окно подтверждения (сек). Повтор ТОГО ЖЕ действия в его пределах = подтверждение.
// Считается по globals->realtime, а НЕ по curtime: curtime обнуляется на смене карты, а
// resetConfirmTime живёт в объекте игрока до дисконнекта — после смены карты отметка из
// прошлой карты оказалась бы «в будущем», и первое же одиночное нажатие прошло бы как
// подтверждение (fail-open, найдено ревью). Отрицательная разница отбивается отдельно.
// Защита ВОЗОБНОВЛЯЕМАЯ: не подтвердил — через секунду простоя она снова взведена, и
// следующее одиночное нажатие опять только предупредит (решение пользователя 21.08;
// прежний вариант тратил ровно одно предупреждение на ран и дальше пропускал молча).
#define KZ_RESET_CONFIRM_WINDOW 1.0f

// Действия, которым нужно раздельное подтверждение сброса. Раздельность — суть требования:
// «нажал рестарт, понял, что случайно» не должно закрываться следующим нажатием ноуклипа.
// Слот РОВНО на действие, общей корзины нет намеренно: пока !stop/!end/!lj/смена команды жили
// в одном слоте, предупреждение по !end закрывалось нажатием !stop в пределах секунды — то
// есть инвариант «подтверждением считается повтор ТОГО ЖЕ действия» внутри корзины не держался
// (найдено ревью cyb.137). Новое действие-инвалидатор = новый слот, а не переиспользование.
enum ResetConfirmAction : u8
{
	RESET_CONFIRM_RESTART = 0,   // безаргументный !r/!restart — единственный «бинд-подобный» рестарт
	RESET_CONFIRM_NOCLIP,        // !noclip
	RESET_CONFIRM_STOP,          // !stop
	RESET_CONFIRM_END,           // !end — телепорт в конец курса
	RESET_CONFIRM_JUMPSTAT_AREA, // !lj — телепорт в jumpstat-зону
	RESET_CONFIRM_TEAM_CHANGE,   // смена команды / уход в спектатор
	RESET_CONFIRM_COUNT,
};
...
public:
	...
	{
		// Восстановленный ран — чужая история подтверждений к нему не относится (и отметка из
		// прошлой карты не должна попасть в окно нового рана).
...
private:
	...
	// Момент первого нажатия по каждому действию, ждущего подтверждения (globals->realtime,
	// не curtime — почему, см. KZ_RESET_CONFIRM_WINDOW).
	// 0 = подтверждения не ждём. Раздельно по действиям намеренно: рестарт и noclip, нажатые
	// подряд, НЕ подтверждают друг друга — подтверждением считается повтор ТОГО ЖЕ действия.
	// См. KZTimerService::CheckSafeguard.
	f64 resetConfirmTime[RESET_CONFIRM_COUNT] {};
...
public:
	...
	// CheckSafeguard — только на "sgReset".
	void ToggleSafeguard();
	void ToggleProSafeguard();
	// Гейт «сброса таймера». action задаёт, чьё подтверждение ждём (см. resetConfirmTime) и
	// попадает ли вызов в окно свободного рестарта: у RESET_CONFIRM_RESTART оно есть, у
	// остальных нет.
	// wantConfirm=false — пропустить подтверждение при ВЫКЛЮЧЕННОМ !sg (блок при включённом
	// остаётся). Для путей с явной целью, которые нельзя выполнить случайно и нельзя повторить
	// за секунду: пункт меню !courses, !course <имя>, !main, !bN. См. KZ::misc::TeleportToCourse.
	bool CheckSafeguard(ResetConfirmAction action, bool showError = true, bool wantConfirm = true);
