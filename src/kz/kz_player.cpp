...
void KZPlayer::OnPhysicsSimulatePost()
{
	...
	if (this->specService->GetSpectatedPlayer())
	{
		...
		// Меню (Task 11) в спектейте за другим игроком БОЛЬШЕ НЕ ЗАКРЫВАЕМ — блок закрытия
		// снят по спеке 2026-09-09-hud-share §6. Он был нашей предосторожностью (8966e80:
		// «нет своей пешки → гасим persistent-состояние»), а не фиксом наблюдённого дефекта:
		// апстрим меню в спектейте не закрывает, сущность меню персональная и от пешки не
		// зависит, а цена предосторожности — игрок не может настроить худ, пока смотрит на
		// чужой. Следствие: этот путь снятия захвата курсора исчез, и страховкой от залипшего
		// курсора остаётся инвариант ниже (hudService->CheckMenuCaptureInvariant()) —
		// захват при закрытом меню виден в логах и снимается принудительно.
		// Блок «мёртв и никого не наблюдает» (ниже) закрытие СОХРАНЯЕТ: там гасится и сама
		// сущность худа, и меню поверх фрикама смысла не несёт.
	}
	...
	else
	{
		...
		// Меню реплея: DrawPanels сюда не доходит, а именно этим кадром обычно кончается
		// просмотр бота (кик бота → цели нет) — закрываем явно, чтобы сущность и флаг не
		// висели до следующего спектейта.
		this->hudService->CloseReplayMenu("no_target");
		...
		// Редактор !hud — та же сущность и тот же захват, закрывается тем же CloseLayoutMenu.
		if (this->hudService->IsLayoutUiOpen())
		{
			// Печатаем ОДИН раз на закрытие: у зрителя два состояния, и это второе —
			// свободная камера (роуминг, m_hObserverTarget невалиден, kz_spec.cpp отдаёт NULL).
			// Без строки зритель, нажавший роум с открытым меню, терял бы его молча, и фича
			// выглядела бы флаки ровно в половине случаев (за игроком — работает, в роуминге —
			// исчезает). Наблюдение за игроком меню НЕ закрывает (ветка выше).
			this->languageService->PrintChat(true, false, "HUD - Menu Closed Spectate");
			this->hudService->CloseLayoutMenu();
		}
	}
	// Инвариант (спека §6): не бывает включённого захвата ввода при закрытом меню. Проверка
	// на живом игроке, потому что путей закрытия шесть и один из них только что убран;
...
void KZPlayer::DisableTurnbinds()
{
	...
	// Turn-бинды разрешены (cyber, решение пользователя 15.08): гасим только когда
	// KZ_AreTurnbindsAllowed() ложен. При разрешённых биндах угол пишем КАЖДЫЙ кадр —
	// иначе после переключения на живом сервере (cvar или подключение к глобальному API)
	// первый же кадр с зажатым биндом швырнёт игрока на yaw, протухший с момента
	// последнего «разрешённого» кадра. В ветке подавления lastValidYaw не трогаем: он
	// там якорь, к которому угол и возвращается.
