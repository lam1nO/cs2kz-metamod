...
struct RunRecorder : public Recorder
{
	...
	// Это единственный фильтр прыжков реплея рана: OnJumpFinish больше ничего не отсекает
	// (#7 — пишем все прыжки, включая невалидные и failstat'ы), а невалидному прыжку FromJump
	// тир не выставляет вовсе, так что порог >= 1 отрежет и его.
