...
// отдельный сетевой info_particle_system, и каждое перебирает KZ::quiet::OnCheckTransmit на
// КАЖДЫЙ CheckTransmit для КАЖДОГО получателя. Постоянный слой один на карту, поэтому его цена
// фиксирована и от числа игроков не зависит.
//
// После перехода на нижнюю грань потолок держит уже не порядок величины, а верхний край:
// при потолке набора в 64 зоны совсем без ограничения вышло бы 256 рёбер (64*4) против 160
// у нынешних сорока. Разница меньше сотни энтити, и ограничение оставлено не ради неё:
// лимит энтити на сервере общий, и его выбирают ещё !zone show (до 576 рёбер) и САМИ зоны —
// упор бьёт не по картинке, а по спавну триггеров (SpawnZone → removed_on_spawn). Плюс
// смысловое: подсветка нужна как ориентир, а не как ёлка из сорока с лишним контуров разом.
//
// Число поднято с 20 до 40 в cyb.123 и это ПРЯМОЕ СЛЕДСТВИЕ перехода на нижнюю грань: зона
// стоит теперь четырёх энтити вместо двенадцати, поэтому 40 зон дают 160 рёбер против 240 у
// прежних двадцати. То есть потолок вырос вдвое, а цена в мире всё равно упала на треть. Сорок
// зон — это двадцать курсов со стартом и финишем: столько бонусов на карте уже редкость.
#define KZ_ZONE_HIGHLIGHT_MAX_ZONES 40
...
// трансмите.
//   * на игрока — 96 рёбер (8 зон). Меньше постоянного слоя намеренно: показ адресный,
//     живёт 12 секунд и нужен, чтобы сориентироваться, а не изучать карту целиком. Сравнивать
//     эти числа с потолком подсветки в зонах бессмысленно: там зона стоит четырёх рёбер
//     (нижняя грань), здесь — двенадцати (полный ящик).
...
// Рёбер в полном контуре AABB. Именованная константа, потому что размер массивов хендлов и
// граница цикла отрисовки обязаны совпадать.
#define KZ_ZONE_BOX_EDGES 12

// Рёбер в нижней грани — периметр на уровне mins.z. Столько рисует ПОСТОЯННЫЙ слой (cyb.123):
// живой отзыв по cyb.122 — «зона светится ещё и вверх, мне надо было только на полу». Габарит
// зоны по высоте остаётся делом администратора, и его показывает !zone show полным ящиком.
// Это ПРЕФИКС той же таблицы рёбер, что и полный контур, а не второй набор: порядок таблицы
// проверяется static_assert'ом рядом с ней в kz_zones.cpp.
#define KZ_ZONE_BOX_BOTTOM_EDGES 4
...
namespace KZ::zones
{
	...
	// Контур AABB беам-частицами, ребро = частица. Штатный kz_showtriggers наши зоны не рисует: он
	...
	// edgeCount — что именно рисуем, и он же размер массива out. Допустимы ровно два значения:
	// KZ_ZONE_BOX_EDGES (полный ящик — !zone show и превью редактора) и KZ_ZONE_BOX_BOTTOM_EDGES
	// (нижняя грань — постоянная подсветка). Параметр, а не вторая функция: набор рёбер один и тот
	// же, у копии разъехались бы порядок таблицы и зануление хвоста.
	// out принимает edgeCount хендлов; на месте несозданных остаётся пустой хендл.
	// Возвращает число созданных рёбер: меньше edgeCount значит «движок не отдал энтити», и
	// вызывающий не вправе считать зону нарисованной.
	i32 DrawBoxEdges(const Vector &mins, const Vector &maxs, const Color &color, const char *targetname, bool ownerOnly, i32 edgeCount,
					 CEntityHandle *out);
