...
public:
	...
	struct PracPoint
	{
		...
		// Курс попытки, к которой относится показание часов (см. pracCourseGUID): !practp
		// переносит его вместе со временем — точка из другой попытки или с чужого курса иначе
		// оставила бы прежний GUID, и финиш печатался бы не на том курсе. 0 = часы стояли
		// (курса у попытки нет) или курс неизвестен — так бывает у точки, забранной у
		// наблюдаемого, который просто ходил по карте.
		u32 courseGUID {};
	};
...
private:
	...
	// в чат на финише. Три источника: вход с замороженным раном (продолжают время рана),
	// !practp (откручивает к показанию точки) и выход из стартовой зоны — с нуля, как начало
	// попытки (OnStartZoneEndTouch).
	f64 pracTime {};
	bool pracTimeRunning {};
	// Курс, к которому относится идущая попытка; 0 = неизвестен (часы пришли с точки, забранной
	// у наблюдаемого). Нужен финишу: попытка началась на одном курсе, а финишных зон на карте
	// много, и без сверки выход из старта главного + забег в финиш бонуса печатал бы время про
	// забег, которого не было.
	u32 pracCourseGUID {};
...
public:
	...
	// Выход из стартовой зоны курса в prac — ЕДИНСТВЕННЫЙ эффект этой зоны в prac (решение
	// пользователя 07.08): запускает prac-часы с нуля, «отсюда пошла попытка». Ни таймера рана,
	// ни курса, ни замороженного рана, ни prac-точек, ни чекпоинтов/tpCount не трогает.
	void OnStartZoneEndTouch(const KZCourseDescriptor *course);
	// Касание финишной зоны курса. true = обработали сами (настоящий TimerEnd звать НЕЛЬЗЯ:
	// в prac рана не существует, а весь сабмит висит на нём). Курс — чтобы печатать финиш только
	// того курса, на котором попытка началась (см. pracCourseGUID).
	bool OnEndZoneTouch(const KZCourseDescriptor *course);
	// Уход в спектатор: prac, замороженный ран и стек точек НЕ теряются, гасим только ноуклип
	// (pawn у обсервера всё равно исчезает).
...
private:
	...
	// и наличие пешки у source проверяет вызывающий, здесь гардов нет. courseGUID — курс той
	// попытки, чьё время передаётся (0 = неизвестен); при timeRunning=false игнорируется.
	void CapturePointFrom(KZPlayer *source, f64 time, bool timeRunning, u32 courseGUID);
