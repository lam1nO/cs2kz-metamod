...
public:
	...
	// возвращается обратно. Заодно убирает из мира выброшенную НАМИ сущность — тем же
	// предикатом ShouldSweep, что и остальные пути уборки (признак «ничья» — отсутствие
	// в инвентарях, а НЕ пустой владелец: у брошенного владелец остаётся).
	void SyncFromHeld();

	// Перевыдать всё выданное. Зовётся ПОСЛЕ RemoveAllItems из UpdatePistol.
	void RegiveGiven();

	// Игрок нажал G. Снимаем слепок инвентаря; всё, что из него уйдёт к следующему
	// usercmd, — выброшенное, и убирается НЕМЕДЛЕННО, не дожидаясь периодической уборки.
	void OnDropCommand();
...
private:
	...
	// held — слепок инвентарей, собранный ОДИН раз вызывающим: иначе на каждый элемент
	// шёл бы полный обход игроков, и признак полноты мог бы разойтись внутри цикла.
	void RemoveDroppedEntity(i32 index, const CUtlVector<CBaseEntity *> &held);
	// Разобрать слепок дропа: что ушло из инвентаря — то выброшено, сносим сразу.
	void SweepDropped();
	...
	// Слепок инвентаря на момент нажатия G. Пуст, пока дропа не было.
	CUtlVector<CHandle<CBaseEntity>> dropWatch {};
};
