...
struct MHUDLayoutPrefs
{
	...
	// ВНИМАНИЕ: с переездом меню на свою страницу (KZ_RPMENU_LAYOUT) рендер полей x/y/size/step/
	// font/outline/background НЕ читает — вся вёрстка карточки живёт в её vcss. Кэш и ключи
	// оставлены намеренно, см. layout.h у RPMENU_DEF_*. ЕДИНСТВЕННОЕ живое поле — scale.
	struct ReplayMenu
	{
		// Масштаб карточки, проценты. Произвольное число сервер в CSS поставить не может,
		// поэтому значение снапится к ступеням RPMENU_SCALE_STEPS (layout/layout.h) и уезжает
		// классом .rp-scale--N на корень страницы.
		i32 scale {100};
		i32 x {};
		i32 y {};
		i32 size {};
...
private:
	...
	struct ReplayMenuPageState
	{
		...
		i32 scale {-1};        // ступень масштаба карточки (класс .rp-scale--N на корне)
