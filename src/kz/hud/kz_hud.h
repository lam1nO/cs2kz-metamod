...
public:
	...
	// получателю: строк, байт, состояние cvar'ов панели и ПЕРЕЧЕНЬ строк (номер, кегль —
	// самый крупный класс в строке, видимая длина в знаках). Источник данных — как в
	// DrawPanels: наблюдаемый при спектейте. Ёмкость панели клиент задаёт в пикселях, серверу
	// она недоступна, и хвост за рамкой движок молча не рисует — сравнение с экраном отличает
	// обрезку от непостроенной строки. Печать в консоль, без перевода: диагностика.
