...
private:
	...
	// не для тактового пути) и не шлётся (centre-канал — надёжный usermessage, слать 128/с
	...
	struct BottomPanelState
	{
		...
		// верха не перерисовало бы низ. Считается ПОСЛЕ содержимого и уступает ему: канал
		// показывает ограниченное число строк, и отступ не имеет права вытеснить сам худ
		// (см. BottomPadLines/kz_hud_bottom_max_lines).
		u8 padLines {};
...
public:
	...
	// Updated (деф.) — кибершоковская панель (BuildVersionCHud + нижняя панель CP/TP);
