...
private:
	...
	// particle-путь идёт живому владельцу (mhudSource = nullptr) и — с cyb.116 —
	// спектатору РЕПЛЕЙ-БОТА in-eye (mhudSource = бот; партиклы свои у спектатора,
	// со своими настройками). Для обычных игроков spectator-путь закрыт: их particle-
	// оверлей у спектатора уезжает за экран при поворотах (регрессия cyb.34/36, см.
	// комментарий в DrawPanels) — у реплей-бота это проверяет канарейка cyb.116.
	// ВАЖНО: это ТОЛЬКО источник данных. Источник НАСТРОЕК (тумблеры/цвета/раскладка) —
	// всегда this->player, см. MHUDSettingsSource().
	KZPlayer *mhudSource {};

	// Игрок, чьи ДАННЫЕ читает MHUD (mhudSource, если задан, иначе сам игрок).
