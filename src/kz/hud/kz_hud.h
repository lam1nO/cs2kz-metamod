...
public:
	...
	// `kz_hud panel` / `!hud panel` — сколько СТРОК и БАЙТ уходит сейчас в центральную
	// HTML-панель этому получателю (источник данных — как в DrawPanels: наблюдаемый при
	// спектейте). Ёмкость панели клиент задаёт в пикселях, серверу она недоступна, и хвост
	// за рамкой движок молча не рисует — сравнение этого числа с числом строк на экране
	// отличает обрезку от непостроенной строки. Печать в консоль, без перевода: диагностика.
