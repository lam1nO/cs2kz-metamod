...
void KZHUDService::SetHudType(int type)
{
	...
	// Тип худа — ГЕЙТ элемента «Прогресс» (layout/prefs.cpp: путь маршрута грузится только под
	// panorama). Без пересчёта префов уход на Standard/Off оставил бы путь загруженным ради
	// элемента, которого в этих типах нет вовсе — сеть и память впустую. Из меню сюда приходят
	// через onPick, который зовёт RefreshLayoutPrefs и сам (повтор безвреден), а вот команды
	// `!hud standard`/`!hud off` — только через этот метод.
