...
// просто накрывает низ, и нижнюю строку не видно (репорт 05.08). Развести их можно только
// отступом — пустыми строками над содержимым нижней панели. Порог — высота верха, начиная
	...
	if (showTimer)
	{
		...
		if (dataSource->hudService->GetTimerParts(tTime, tRunning))
		{
			...
			if (notRunning)
			{
				...
				{
					// Часы стоят: действующей попытки нет (ноуклип обнулил её / prac без рана —
					// в свободном prac часы стоят всегда, стартовая зона их не заводит с 07.08).
					// Плейсхолдер той же ширины, что «MM:SS.CC», DIM — тот же идиом, что у пустых
					// PB/WR ниже. Белое 00:00.00 читалось бы как «время ноль, часы вот-вот
					// пойдут», а они не пойдут ни от чего, кроме !practp на точку со временем.
					tTime = "--:--.--";
