										   ...
										   const char *language)
{
	...
	// Кибершок-вид «две панели»: клавиши + CP/TP отделяются от инфо-блока ПУСТОЙ строкой-зазором.
	// Гипотеза — фон center-HTML CS2 обтягивает текст построчно, поэтому пустая строка рвёт плашку
	// на два визуальных «бокса» (проверяется вживую). Если фон окажется сплошным — станет просто
	// вертикальным отступом (тогда добавим видимую линию-разделитель). Только полный режим (в
	// компакте второй панели нет) и только когда во второй панели реально есть контент.
	...
	{
		html += "<br>"; // лишний <br> → пустая строка-зазор (addLine ниже добавит свой <br>)
	}
