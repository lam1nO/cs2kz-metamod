...
static_function f64 PollCrosshairCvars(CPlayerUserId userID)
{
	...
	// Дефолт mhudCrosshair — true, тот же, что у читателя в layout/prefs.cpp: с false опрос
	// конваров не стартовал у игрока, которому худ прицел уже рисует (два источника одного
	// дефолта — этот класс дефекта в трансе ловился дважды).
