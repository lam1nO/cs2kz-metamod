// Таблицы классов panorama, перенесённые из апстримного src/kz/option/menu/tables.cpp:
// они соответствуют сгенерированным .vcss в аддоне 3469155349 и меняются только вместе с ним.
...
static_global const PanoramaFontDef PANORAMA_FONTS[] =
{
...
static_global const i32 PANORAMA_FONT_COUNT = KZ_ARRAYSIZE(PANORAMA_FONTS);
...
// Градиент хранится как маркер-цвет: alpha == 1, red == индекс градиента. У сплошных цветов
// alpha всегда 255.
static_function bool IsGradient(const Color &c)
{
	return c.a() == 1;
}

static_function Color MakeGradient(i32 index)
{
	return Color((u8)index, 0, 0, 1);
}

static_function i32 GetGradientIndex(const Color &c)
{
	return c.r();
}
...
// Листы определяют не каждое значение: позиции — шаг 1 внутри +-100 и шаг 5 до +-500,
// размеры — шаг 1 до 100, дальше 5. Снап не даёт префу назвать несуществующий класс.
