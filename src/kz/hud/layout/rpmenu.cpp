...
namespace
{
	...
	//
	// ШАГ 1 %, А НЕ МЕЛЬЧЕ — ЭТО ФИКС КРАША, А НЕ ВКУСОВЩИНА. На шаге 0.25 % файл позиций
	// имел 802 правила с дробными процентами, и клиент CS2 падал МОЛЧА прямо на загрузке
	// страницы: ни строки в консоли, ни дампа, ни одной панорамной ошибки. Локализовано
	// бисектом на канарейке 17.09.2026 — та же страница с тем же CSS, но БЕЗ файла позиций,
	// не падала. У апстрима (positions.vcss чужого аддона) шаг 1 % и целые проценты.
	// Менять шаг обратно НЕЛЬЗЯ; генератор — tools/build_rpmenu_positions.py в аддоне.
	//
