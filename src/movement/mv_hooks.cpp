...
{
	// ДИАГНОСТИЧЕСКАЯ СБОРКА (23.09.2026): детуры движения НЕ ставятся. Нужна, чтобы
	// отделить «падает из-за наших хуков» от «падает само по себе»: после апдейта CS2
	// 25470087 сервер умирал между загрузкой карты и первым игровым тиком, внутри функции
	// движка, которую мы не детурим. В релиз не мержить.
	Msg("[CS2KZ] ДИАГНОСТИКА: детуры движения отключены\n");
	return true;
