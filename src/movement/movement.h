...
public:
	...
	// Метка касания для КЛАССИФИКАЦИИ ВВОДА игрока. landingTimeActual в ветке предсказания
	// (reverse bug ниже) уезжает в БУДУЩЕЕ относительно такта, в котором движок уже поставил
	// игрока на землю: при малой |vz| корень в квадратном уравнении даёт десятки мс вперёд.
	// Сравнивать клик с такой меткой нельзя — он арифметически всегда «до касания».
	// Здесь метка никогда не в будущем: точное субтиковое время трейс-ветки сохраняется,
	// предсказание в будущее срезается до такта движка.
	f32 landingTimeInput {};
	// Диагностика касания для леджера прыжков (поведение не гейтят).
	f32 landingDiffZ {-1.0f};      // зазор до пола в момент регистрации, юниты (-1 = не считали)
	i32 landingTimeSource {-1};    // -1=касания не было, 0=трейс-фракция, 1=уже стоит/ледж, 2=предсказание
	// Максимум высоты в воздухе с момента отрыва — только для леджера прыжков, копит
	// режим (под своим отладочным cvar). Гасится на отрыве и на телепорте, поэтому не
	// склеивает полёты через чекпоинт.
	f32 airMaxZ {};
